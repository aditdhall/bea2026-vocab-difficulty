"""
Phase 4, Exp 0.5: Target Variable Scaling
==========================================
Z-score normalizes GLMM_score per L1 before training.
Computes mean/std on training data only, inverse-transforms predictions.

CLOSED TRACK: Per-L1 training. Encoder-only. Compliant.
Requires GPU. ~30 min on A100.
"""
...
def main():
    ...
    print(f"{'='*60}")
    print(f"  EXP 0.5: Target Variable Scaling (xlm-roberta-large hybrid)")
    print(f"{'='*60}")
    ...
    for l1 in ['es', 'de', 'cn']:
        ...
        y_mean = train_df['GLMM_score'].mean()
        y_std = train_df['GLMM_score'].std()
        print(f"  GLMM mean={y_mean:.4f}, std={y_std:.4f}")

        # Scale target
        train_df_scaled = train_df.copy()
        train_df_scaled['GLMM_score'] = (train_df['GLMM_score'] - y_mean) / y_std
        ...
        train_ds = VocabularyDataset(texts_tr, X_tr_f, train_df_scaled['GLMM_score'].values, tokenizer)
        ...
        preds_scaled = predict_hybrid(model, dev_loader, device)

        # Inverse transform
        preds_original = preds_scaled * y_std + y_mean
        rmse = np.sqrt(np.mean((dev_df['GLMM_score'].values - preds_original)**2))
        print(f"  {l1} RMSE (scaled training): {rmse:.4f} (baseline: {CLOSED_BASELINES[l1]})")

        out = pd.DataFrame({'item_id': dev_df['item_id'], 'prediction': preds_original})
        out.to_csv(f'{PRED_DIR}/exp05_scaled_{l1}_dev.csv', index=False)
